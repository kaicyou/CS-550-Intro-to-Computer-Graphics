...
const char *WINDOWTITLE = { "Final Project -- Haoxiang Wang" };
...
Display( )
{
	...
	//disco lights
	SetSpotLight(GL_LIGHT2, 10., -sqrt(42.5*42.5 - 27.*27.) - 7.5, 10., DiscoLight1Dirx, DiscoLight1Diry, DiscoLight1Dirz, 1., 0., 0.);
	SetSpotLight(GL_LIGHT3, 10., -sqrt(42.5*42.5 - 27.*27.) - 7.5, -10., DiscoLight2Dirx, DiscoLight2Diry, DiscoLight2Dirz, 0., 1., 0.);
	SetSpotLight(GL_LIGHT4, -10., -sqrt(42.5*42.5 - 27.*27.) - 7.5, -10., DiscoLight3Dirx, DiscoLight3Diry, DiscoLight3Dirz, 0., 0., 1.);
	...
	if (SnowSwitch == 1)
	{
		glColor3f(1., 1., 1.);
			for (int j = 0; j < snownum; j++)
			{
				...
				{
					snowpos[q][j][0] += Time * snowmove[q][j][0];
					snowpos[q][j][1] += Time * snowfall[q][j];
					snowpos[q][j][2] += Time * snowmove[q][j][1];
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., "Solar System in a Crystal Ball" );
